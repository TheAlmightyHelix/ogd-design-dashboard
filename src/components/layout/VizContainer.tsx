...
const VizContainer = React.forwardRef<HTMLDivElement, VizContainerProps>(
  (
    ...
    const prevGameDataIdsRef = useRef(gameDataIds);
    const prevVizTypeRef = useRef(vizType);

    const canExportChart = containerMode === 'viz' && Boolean(dataset);

    const getExportFilename = () =>
      buildChartImageFilename(
        title || getDefaultChartTitle(gameDataIds, vizType),
        vizType,
        chartId,
      );

    const handleDownloadImage = async () => {
      if (!chartCaptureRef.current || isExporting) return;
      ...
      try {
        ...
        trackEvent('chart_image_download', {
          viz_type: vizType,
          chart_id: chartId,
        });
      } catch (error) {
        console.error('Failed to download chart image:', error);
        window.alert('Failed to download chart image.');
      } finally {
    ...
    const handleCopyImage = async () => {
      ...
      setIsExporting(true);
      try {
        await copyChartImageToClipboard(chartCaptureRef.current);
        trackEvent('chart_image_copy', {
          viz_type: vizType,
          chart_id: chartId,
        });
      } catch (error) {
        ...
        window.alert(
          error instanceof Error
            ? error.message
            : 'Failed to copy chart image to clipboard.',
        );
      } finally {
        setIsExporting(false);
      }
    ...
    useEffect(() => {
      if (!title) {
        prevGameDataIdsRef.current = gameDataIds;
        prevVizTypeRef.current = vizType;
        return;
      }

      const prevIds = prevGameDataIdsRef.current;
      const prevVizType = prevVizTypeRef.current;
      const idsChanged = prevIds.join(',') !== gameDataIds.join(',');
      const vizTypeChanged = prevVizType !== vizType;

      if (
        isAutoGeneratedTitle(title, gameDataIds, vizType, getDatasetByID) ||
        (idsChanged &&
          isAutoGeneratedTitle(title, prevIds, vizType, getDatasetByID)) ||
        (vizTypeChanged &&
          isAutoGeneratedTitle(
            title,
            gameDataIds,
        ...
        >
          <div className="flex-shrink-0 min-w-0 w-full">
            <Input
              ...
              placeholder={
                getDefaultChartTitle(gameDataIds, vizType) || 'Title...'
              }
              debounce
            />
          </div>
    ...
    return (
      ...
      >
        ...
        <div className={CHART_CONTROL_GROUP_CLASS} data-chart-export-exclude>
          <button
          ...
          {canExportChart && (
            <>
              <button
                className={`${CHART_CONTROL_BUTTON_CLASS} cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed`}
